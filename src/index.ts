import { type ConfigPlugin, IOSConfig, createRunOncePlugin, withDangerousMod, withXcodeProject } from '@expo/config-plugins'
import type { ExpoConfig } from '@expo/config-types'
import { copyFileSync, ensureDirSync, readdir } from 'fs-extra'
import * as path from 'node:path'

function withCustomAssetsAndroid(config: ExpoConfig, props: { assetsPaths: string[]; ignoredPattern?: string }) {
...
function withCustomAssetsIos(config: ExpoConfig, props: { assetsPaths: string[]; assetsDirName?: string; ignoredPattern?: string }) {
...
const withCustomAssets: ConfigPlugin<{ assetsPaths: string[]; assetsDirName?: string; ignoredPattern?: string }> = (config, props) => {
